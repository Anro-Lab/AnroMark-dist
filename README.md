...
Public distribution channel for **AnroMark Pro**.

## Purpose

This repository is a write-only distribution endpoint. It hosts the Windows desktop installer artifacts that both the AnroMark Pro Launcher and the in-app `electron-updater` auto-update system use.
...
For any tag `vX.Y.Z`:
- AI-Desktop source tag: `Anro-Lab/AnroMark-win@vX.Y.Z`
...
└── win-unpacked/
    ├── AnroMark Pro.exe      ← main Windows executable
    ├── locales/
    ├── resources/
...
### 2. `AnroMark-Pro-Setup-X.Y.Z.exe` — NSIS Installer
...
version: X.Y.Z
files:
  - url: AnroMark-Pro-Setup-X.Y.Z.exe
    sha512: <base64-sha512>
path: AnroMark-Pro-Setup-X.Y.Z.exe
sha512: <base64-sha512>
